import csv
from datetime import datetime

# Dictionary to store student information
students = {}

def load_students():
    try:
        with open('students.csv', 'r') as file:
            reader = csv.DictReader(file)
            for row in reader:
                roll_number = row['Roll Number']
                students[roll_number] = {'name': row['Name'], 'attendance': {}}
        print("Students loaded successfully.")
    except FileNotFoundError:
        print("No saved student data found.")

def load_attendance():
    try:
        with open('attendance.csv', 'r') as file:
            reader = csv.DictReader(file)
            for row in reader:
                roll_number = row['Roll Number']
                if roll_number in students:
                    students[roll_number]['attendance'][row['Date & Time']] = row['Status']
        print("Attendance records loaded successfully.")
    except FileNotFoundError:
        print("No saved attendance data found.")

def save_students():
    with open('students.csv', 'w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(['Roll Number', 'Name'])
        for roll_number, student in students.items():
            writer.writerow([roll_number, student['name']])
    print("Student data saved successfully.")

def save_attendance():
    with open('attendance.csv', 'w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(['Roll Number', 'Name', 'Date & Time', 'Status'])
        for roll_number, student in students.items():
            for datetime_marked, status in student['attendance'].items():
                writer.writerow([roll_number, student['name'], datetime_marked, status])
    print("Attendance data saved successfully.")

def add_student():
    roll_number = input("Enter roll number: ").upper()
    name = input("Enter student name: ").title()
    if roll_number in students:
        print("Student already exists.")
    else:
        students[roll_number] = {'name': name, 'attendance': {}}
        save_students()  # Save the updated student list
        print("Student added successfully.")

def mark_attendance():
    roll_number = input("Enter roll number: ").upper()
    if roll_number not in students:
        print("Student not found.")
        return
    datetime_now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    date = datetime.now().strftime("%Y-%m-%d")
    
    if date in students[roll_number]['attendance']:
        print("Attendance for today is already marked.")
    else:
        status = input("Enter status (present/absent): ").lower()
        if status in ['present', 'absent']:
            students[roll_number]['attendance'][datetime_now] = status
            save_attendance()  # Save the updated attendance data
            print(f"Attendance marked successfully at {datetime_now}.")
        else:
            print("Invalid status. Please enter 'present' or 'absent'.")

def view_attendance_for_student():
    roll_number = input("Enter roll number: ").upper()
    if roll_number not in students:
        print("Student not found.")
        return
    student = students[roll_number]
    print(f"Attendance for {student['name']} (Roll Number: {roll_number}):")
    for datetime_marked, status in student['attendance'].items():
        print(f"Date & Time: {datetime_marked}, Status: {status}")

def view_attendance_for_all():
    if not students:
        print("No students found.")
        return
    print("Attendance for all students:")
    for roll_number, student in students.items():
        print(f"\nStudent Name: {student['name']}, Roll Number: {roll_number}")
        for datetime_marked, status in student['attendance'].items():
            print(f"Date & Time: {datetime_marked}, Status: {status}")

def main():
    load_students()
    load_attendance()
    while True:
        print("\nStudent Attendance System")
        print("1. Add New Student")
        print("2. Mark Attendance")
        print("3. View Attendance for a Specific Student")
        print("4. View Attendance for All Students")
        print("5. Exit")
        
        choice = input("Enter your choice (1-5): ")
        if choice == '1':
            add_student()
        elif choice == '2':
            mark_attendance()
        elif choice == '3':
            view_attendance_for_student()
        elif choice == '4':
            view_attendance_for_all()
        elif choice == '5':
            save_students()  # Save student data before exiting
            save_attendance()  # Save attendance data before exiting
            print("Exiting the system.")
            break
        else:
            print("Invalid choice. Please enter a number between 1 and 5.")

if __name__ == "__main__":
    main()
